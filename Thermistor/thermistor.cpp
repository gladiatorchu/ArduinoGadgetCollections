/*
  ...
  Modification Record:
  20210717 - Modification of the above library to add support to Pull-up configuration
...
double Thermistor::adcToK(double adc) const {
	double resistance=0;
	// handle different connection configuration
	if (_connectConfig){
		// Pull-up configuration
		resistance = -1.0 * (_seriesResistor * (_analogReference * adc - _vcc * _adcMax)) / (_analogReference * adc);
	}else{
		// Pull-down configuration
		resistance = -1.0 * (_analogReference * _seriesResistor * adc) / (_analogReference * adc - _vcc * _adcMax);
	}
